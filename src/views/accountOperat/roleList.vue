      ...
      <div class="contain-vessel">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            header-align="center"
            v-show="powerBtn.addBtn || powerBtn.deleteBtn"
          >
...
<script>
  ...
  export default {
    created() {
      ...
      if(power.indexOf("addRole")<0) this.powerBtn.addBtn=false;
      if(power.indexOf("deleteRole")<0) this.powerBtn.deleteBtn=false;
      if(power.indexOf("updateRole")<0) this.powerBtn.alertBtn=false;
    },
    data() {
      return {
        loading:false,
    ...
    methods: {
      modifyRole(row) {
        this.$router.push({
          path: `/userSystem/roleedit`,
          query: {id: row.id}
        })
      },
      getList() {
        this.loading=true;
        request.get('/api/auth/v1/operation/role/list', {
          params: {
      ...
      deleteRole(row) {
        ...
        this.$confirm('此操作将删除该角色, 是否继续?', '提示', {
          ...
          request.delete(`/api/auth/v1/operation/role/${row.id}`).then((res) => {
