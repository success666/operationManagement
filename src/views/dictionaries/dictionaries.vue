<template>
  <div class="dictionaries">
    <el-row class="title">
      <el-col :span="14">
        <h3 class="title_name">字典管理({{formType}})</h3>
      </el-col>
    ...
    <el-row class="dis_select" v-if="!isSelect">
      <el-col :span="6">
        <label class="dis_select_label">字典键值</label>
        <el-input size="small" class="dis-select_value" v-model="search.value"></el-input>
      </el-col>
      <el-col :span="6">
        <label class="dis_select_label">字典标签</label>
        <el-input size="small" class="dis-select_value" v-model="search.label"></el-input>
      </el-col>
      <el-col :span="4">
    ...
    <el-row class="dis_table" :style="{'min-height':tableHeight}">
      ...
      >
        ...
        <el-table-column property="value" label="字典键值" width="150" align="center"></el-table-column>
        <el-table-column property="label" label="字典标签" width="150" align="center"></el-table-column>
        <el-table-column property="sort" label="字典排序" width="150" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    cellClick(row, column, cell, event) {
      if (column.property == "description" || column.property == "type") {
        this.search[column.property] = row[column.property];
        this.selectData();
      }
    },
    ...
    handleStart(index, row) {
      request
        .post("/api/auth/v1/dict/discontinue/" + row.id, {
          delFlag: 0
        })
        .then(res => {
          this.selectData();
  ...
  watch: {
    $route(to, form) {
      if (to.name == "Dictionaries" && to.params.type == "update") {
        this.formType = to.params.formType;
        this.selectData();
      }
    }
  },
...
function getFormData(params) {
  params.type = this.$route.params.formType;
  request
    .get("/api/auth/v1/dict/findList", {
      params
    })
    .then(res => {
