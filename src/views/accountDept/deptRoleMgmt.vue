...
<script>
  ...
  export default {
    created() {
      ...
      if(power.indexOf("addMgmtDeptRole")<0) this.powerBtn.addBtn=false;
      if(power.indexOf("deleteMgmtDeptRole")<0) this.powerBtn.deleteBtn=false;
      if(power.indexOf("updateMgmtDeptRole")<0) this.powerBtn.alertBtn=false;
    },
    data() {
      return {
        loading:false,
    ...
    methods: {
      modifyRole(row) {
        this.$router.push({
          path: `/mgmtDeptAccount/deptroleedit`,
          query: {id: row.id}
        })
      },
      getList() {
        this.loading=true;
        request.get('/api/auth/v1/mgmtDept/role', {
          params: {
      ...
      deleteRole(row) {
        ...
        this.$confirm('此操作将删除该角色, 是否继续?', '提示', {
          ...
          request.delete(`/api/auth/v1/mgmtDept/role/${row.id}`).then((res) => {
      ...
      addRole() {
        this.$router.push({
          path: '/mgmtDeptAccount/deptroleadd'
        })
      },
      getCurrent(current) {
        console.log(current);
